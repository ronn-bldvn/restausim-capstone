                    ...
                    <td style="padding:20px;">
                        ...
                        <div class="activity-card" style="background-color:#FDFDFD; border:1px solid #E0E0E0; border-radius:6px;">
                            <div style="padding:24px;">
                                ...
                                <div class="description">{!! nl2br(e($activity->description)) !!}</div>
                                <div class="due-date">
                                    @if($activity->due_date)
                                        Due Date: <span class="date">{{ \Carbon\Carbon::parse($activity->due_date)->timezone('Asia/Manila')->format('F j, Y - g:i A') }}</span>
                                    @else
                                        <span style="color:#888888;">No deadline set.</span>
                                    @endif
                                </div>
                                <div class="posted-info">
                                    Posted on {{ \Carbon\Carbon::parse($activity->created_at)->timezone('Asia/Manila')->format('M j, Y \a\t g:i A') }} by <span class="instructor">{{ $faculty->name ?? 'Instructor' }}</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div style="margin-top:24px; text-align:center;">
                            ...
                            class="activity-button">Click Here to See the Activity</a>
                        </div>
